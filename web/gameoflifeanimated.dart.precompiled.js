...
$$.BoundClosure$3 = [X, {"": "BoundClosure;_self,__js_helper$_target,_receiver,__js_helper$_name",
...
InputElement: {"": "HtmlElement;disabled%,value=", $isElement: true, "%": "HTMLInputElement"},
...
main: function() {
  ...
  H.setRuntimeTypeInfo(t1, [null]);
  t3 = new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(X.createBoard$closure), t1._useCapture);
  ...
  t1 = new W._EventStreamSubscription(0, t3._target, t3._eventType, W._wrapZone(X.startRunning$closure), t3._useCapture);
  ...
  t1 = new W._EventStreamSubscription(0, t2._target, t2._eventType, W._wrapZone(X.stopRunning$closure), t2._useCapture);
  H.setRuntimeTypeInfo(t1, [H.getRuntimeTypeArgument(t2, "_EventStream", 0)]);
  t1._tryResume$0();
},

createBoard: function(e) {
  var rowsInput, columnsInput, circles, t1, rows, columns, i, j, t2, t3;
  rowsInput = document.querySelector("#rows");
  columnsInput = document.querySelector("#columns");
  circles = document.querySelector("#board");
  t1 = J.get$children$x(circles);
  t1.clear$0(t1);
  rows = H.Primitives_parseInt(J.get$value$x(rowsInput), null, null);
  columns = H.Primitives_parseInt(J.get$value$x(columnsInput), null, null);
  $.board = X.Board$(rows, columns);
  ...
  J.set$disabled$x($.runButton, false);
},

startRunning: function(_) {
  $.timer = P.Timer_Timer$periodic($.get$ONE_SECOND(), new X.startRunning_closure());
  X.flipButtonState();
},

stopRunning: function(_) {
  $.timer.cancel$0();
  X.flipButtonState();
},

flipButtonState: function() {
  var t1, t2;
  t1 = $.runButton;
  t2 = J.getInterceptor$x(t1);
  t2.set$disabled(t1, t2.get$disabled(t1) !== true);
  t1 = $.cancelButton;
  t2 = J.getInterceptor$x(t1);
  t2.set$disabled(t1, t2.get$disabled(t1) !== true);
},

startRunning_closure: {"": "Closure;",
  call$1: function(_) {
    var t1 = $.board;
    t1._eachCell$1(t1.get$_markNeighbors());
    t1._eachCell$1(t1.get$_enforceRules());
    return;
  },
  $is_args1: true
},

Circle: {"": "Object;c,filled<,neighborCount?",
  _flipState$1: function(e) {
    var t1 = this.c;
    if (this.filled) {
      J.set$background$x(t1.style, "#FFFFFF");
      this.filled = false;
    } else {
      J.set$background$x(t1.style, "#BADA55");
      this.filled = true;
    }
    return;
  },
  get$_flipState: function() {
    return new H.BoundClosure$1(this, X.Circle.prototype._flipState$1, null, "_flipState$1");
  },
  Circle$3: function($parent, $top, left) {
    var t1, t2, t3;
    t1 = this.c;
    t1.className = "circle";
    t2 = t1.style;
    J.getInterceptor$x(t2).set$position(t2, "absolute");
    C.CssStyleDeclaration_methods.set$top(t2, "" + $top + "px");
    C.CssStyleDeclaration_methods.set$left(t2, "" + left + "px");
    t1.toString;
    t2 = new W._ElementEventStreamImpl(t1, C.EventStreamProvider_click._eventType, false);
    H.setRuntimeTypeInfo(t2, [null]);
    t3 = this.get$_flipState();
    ...
    t3._tryResume$0();
    t3 = J.get$children$x($parent);
    t3.add$1(t3, t1);
  },
  static: {
Circle$: function($parent, $top, left) {
  var t1 = new X.Circle(document.createElement("div", null), false, 0);
  t1.Circle$3($parent, $top, left);
  return t1;
}}

},

Board: {"": "Object;length>,width,_board",
  _eachCell$1: function(visitor) {
    var i, t1, j;
    i = 0;
    while (true) {
      t1 = this.length;
      ...
      while (true) {
        ...
        if (j >= t1.length)
          throw H.ioore(t1, j);
        visitor.call$3(t1[j], i, j);
        ++j;
      }
      ++i;
    }
  },
  _markNeighbors$3: function(c, i, j) {
    var t1 = {};
    t1.neighbors_0 = 0;
    H.IterableMixinWorkaround_forEach($.get$Board__VISIT_ORDER(), new X.Board__markNeighbors_closure(t1, this, i, j));
    c.set$neighborCount(t1.neighbors_0);
  },
  get$_markNeighbors: function() {
    return new X.BoundClosure$3(this, X.Board.prototype._markNeighbors$3, null, "_markNeighbors$3");
  },
  _itemAt$2: function(i, j) {
    var t1 = J.getInterceptor$n(i);
    if (t1.$lt(i, 0) || t1.$gt(i, J.$sub$n(this.length, 1)))
      return;
    t1 = J.getInterceptor$n(j);
    if (t1.$lt(j, 0) || t1.$gt(j, J.$sub$n(this.width, 1)))
      return;
    t1 = this._board;
  ...
  _enforceRules$3: function(c, i, j) {
    var neighbors;
    if (c.get$filled()) {
      neighbors = c.neighborCount;
      if (neighbors < 2 || neighbors > 3) {
        J.set$background$x(c.c.style, "#FFFFFF");
        c.filled = false;
      }
    } else if (c.neighborCount === 3) {
      J.set$background$x(c.c.style, "#BADA55");
      c.filled = true;
    }
    c.neighborCount = 0;
  },
  get$_enforceRules: function() {
    return new X.BoundClosure$3(this, X.Board.prototype._enforceRules$3, null, "_enforceRules$3");
  },
  Board$2: function(rows, columns) {
    var t1, i, t2;
    this.length = rows;
    this.width = columns;
    t1 = P.List_List(rows, [J.JSArray, X.Circle]);
    H.setRuntimeTypeInfo(t1, [[J.JSArray, X.Circle]]);
    this._board = t1;
    if (typeof rows !== "number")
      throw H.iae(rows);
    i = 0;
    for (; i < rows; ++i) {
      t1 = this._board;
      t2 = P.List_List(columns, X.Circle);
      t2.$builtinTypeInfo = [X.Circle];
      if (i >= t1.length)
        throw H.ioore(t1, i);
      t1[i] = t2;
    }
  ...
  static: {
"": "Board__VISIT_ORDER",
Board$: function(rows, columns) {
  var t1 = new X.Board(null, null, null);
  t1.Board$2(rows, columns);
  return t1;
}}

},

Board__markNeighbors_closure: {"": "Closure;box_0,this_1,i_2,j_3",
  call$1: function(offsets) {
    var t1, dy, neighbor, t2;
    t1 = J.getInterceptor$asx(offsets);
    dy = t1.$index(offsets, 0);
    neighbor = this.this_1._itemAt$2(J.$add$ns(this.i_2, t1.$index(offsets, 1)), J.$add$ns(this.j_3, dy));
    if (null != neighbor) {
      t1 = this.box_0;
      t2 = t1.neighbors_0;
      t1.neighbors_0 = t2 + (neighbor.get$filled() ? 1 : 0);
    }
  },
  $is_args1: true
}},
...
init.globalFunctions.createBoard$closure = X.createBoard$closure = new H.Closure$1(X.createBoard, "createBoard$closure");
init.globalFunctions.startRunning$closure = X.startRunning$closure = new H.Closure$1(X.startRunning, "startRunning$closure");
init.globalFunctions.stopRunning$closure = X.stopRunning$closure = new H.Closure$1(X.stopRunning, "stopRunning$closure");
...
X.Circle.$isCircle = true;
X.Circle.$isObject = true;
...
Isolate.$lazy($, "ONE_SECOND", "ONE_SECOND", "get$ONE_SECOND", function() {
  return P.Duration$(0, 0, 0, 0, 0, 1);
});
Isolate.$lazy($, "_VISIT_ORDER", "Board__VISIT_ORDER", "get$Board__VISIT_ORDER", function() {
...
function dart_precompiled($collectedClasses) {
  ...
  function startRunning_closure() {
  }
  startRunning_closure.builtin$cls = "startRunning_closure";
  if (!"name" in startRunning_closure)
    startRunning_closure.name = "startRunning_closure";
  $desc = $collectedClasses.startRunning_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  startRunning_closure.prototype = $desc;
  function Circle(c, filled, neighborCount) {
    this.c = c;
    this.filled = filled;
    this.neighborCount = neighborCount;
  }
  Circle.builtin$cls = "Circle";
  if (!"name" in Circle)
    Circle.name = "Circle";
  $desc = $collectedClasses.Circle;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Circle.prototype = $desc;
  Circle.prototype.get$filled = function() {
    return this.filled;
  };
  Circle.prototype.set$neighborCount = function(v) {
    return this.neighborCount = v;
  };
  function Board(length, width, _board) {
    this.length = length;
    this.width = width;
    this._board = _board;
  }
  ...
  Board.prototype.get$length = function(receiver) {
    return this.length;
  };
  ...
  function Board__markNeighbors_closure(box_0, this_1, i_2, j_3) {
    this.box_0 = box_0;
    this.this_1 = this_1;
    this.i_2 = i_2;
    this.j_3 = j_3;
  }
  Board__markNeighbors_closure.builtin$cls = "Board__markNeighbors_closure";
  if (!"name" in Board__markNeighbors_closure)
    Board__markNeighbors_closure.name = "Board__markNeighbors_closure";
  $desc = $collectedClasses.Board__markNeighbors_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Board__markNeighbors_closure.prototype = $desc;
